        ...
        success: function(data) {
            if (!data) {
                if (location.pathname.indexOf("/admin/main") != -1) {
                    ...
                    window.location.reload(); // 如果不是就刷新页面,自动跳转到登录页面
                }
            } else {
                sysAdminSessionInfo = data; // 赋值
                $("#adminNameTXT").text("欢迎您：" + data.sysAdminEntity.adminName);
            }
        },
        error: function(data) {
            location.href = basePath + "login.jsp";
...
function initLeftMenue(data) {
    ...
    menues.each(function() {
        var menue = $(this);
        var bool = AuthorityIsHas(data, menue.text());
        if (bool) {
            menue.show();
        } else {
            menue.hide();
